...
def main(args, hieunq_images_dir, hieunq_prefix):
    args.image_dir = hieunq_images_dir
    ...
    #############------SAVE RESULTS------#####################
    hieunq_path = "/raid/kientdt/shared_drive_cv/ocr/kientdt/PaddleOCR/inference_results" + hieunq_prefix
    if not os.path.exists(hieunq_path):
        os.makedirs(hieunq_path)
    annotation_format = "<image_path>\t<prediction>\n"
    hieunq_results = []
    for ino in tqdm(range(len(img_list))):
        # logger.info(
        #     "Predicts of {}:{}".format(valid_image_file_list[ino], rec_res[ino])
        # )
        image_path = valid_image_file_list[ino]
        pred_text = rec_res[ino][0]
        hieunq_results.append((image_path, pred_text))
    with open(os.path.join(hieunq_path, "pred.txt"), mode="a", encoding="utf-8") as file:
        for element in hieunq_results:
            image_path, pred_text = element
            line = annotation_format.replace("<image_path>", image_path).replace("<prediction>", pred_text)
            file.write(line)
    print("DONE!")
    if args.benchmark:
...
if __name__ == "__main__":
    bigfolder = "./TestOCRData"
    print(os.listdir(bigfolder))
    for index in [1, 2, 3, 4]:
        batch_folder = "/batch_" + str(index)
        for font_folder in os.listdir(bigfolder + batch_folder):
            images_dir = bigfolder + batch_folder + "/" + font_folder + "/train"
            assert os.path.exists(images_dir)
            print("Inference on: ", images_dir)
            main(utility.parse_args(), images_dir, batch_folder + "/" + font_folder)
